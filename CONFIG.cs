...
namespace CS_MYSQL_DATA
{
    public class CONFIG
    {
        ...
        public CONFIG(string _file_name, string _dir_path = "etc")
        {
            ...
            path = Path.Combine(home_path, dir_path, file_name);
            ...
            if (Directory.Exists(dir_path) == false)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), file_name);
            ...
            try
            {
                mutex = new Mutex(false, path.Replace("\\", "_"));
            }
            catch (Exception ex)
            {
                Console.WriteLine("[WARNING] {0}: {1}", GetType().Name, ex.Message);
